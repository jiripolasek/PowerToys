...
public sealed partial class AppListItem : ListItem, IPrecomputedListItem
{
    ...
    private Task<Details> BuildDetails()
    {
        ...
        // Icon
        var heroIconPath = !string.IsNullOrEmpty(_app.JumboIconPath)
            ? _app.JumboIconPath
            : !string.IsNullOrEmpty(_app.IcoPath)
                ? _app.IcoPath
                : _app.ExePath;
        var heroImage = string.IsNullOrEmpty(heroIconPath)
            ? null
            : new IconInfo(_app.IsPackaged ? heroIconPath : $"{JumboAppIconPrefix}{heroIconPath}");
    ...
    private static IconInfo CreateIcon(AppItem app, bool useThumbnails)
    {
        ...
        return new IconInfo(!app.IsPackaged && useThumbnails ? $"{AppIconPrefix}{iconPath}" : iconPath);
    }
