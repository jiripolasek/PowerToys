...
public class IconPathConverterTests
{
    ...
    [TestMethod]
    public void GeneratedIconIsPreparedAsThemeSpecificSvgData()
    {
        ...
        Assert.IsNotNull(prepared.SvgData);
        var svg = Encoding.UTF8.GetString(prepared.SvgData);
        StringAssert.Contains(svg, "fill=\"#60CDFF\"");
        StringAssert.Contains(svg, "<path d=");
        Assert.IsFalse(svg.Contains("<text", StringComparison.Ordinal));
        Assert.IsFalse(IconPathConverter.TryCreateIconSourceSynchronously(prepared, out var source));
