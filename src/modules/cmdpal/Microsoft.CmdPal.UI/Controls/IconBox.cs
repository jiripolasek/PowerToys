...
public partial class IconBox : ContentControl
{
    ...
    private IconRequestReason _refreshReason;
    private IconRequestMeasurement _activeRequestDiagnostics;
    private IIconRequestDemand? _activeRequestDemand;
    private bool _resolvedSourceExpectsImage;

    // ImageIconSource does not render through IconSourceElement. Keep a stable Grid and
    // alternate ordinary Image controls so recycled rows never mutate the visible slot.
    ...
    public event TypedEventHandler<IconBox, SourceRequestedEventArgs>? SourceRequested
    {
        add
        {
            ...
            {
                _needsRefresh = SourceKey is not null;
                if (SourceKey is not null)
                {
                    _refreshReason |= IconRequestReason.HandlerAttached;
                }
    ...
    private void OnActualThemeChanged(FrameworkElement sender, object args)
    {
        ...
        if (SourceKey is not null)
        {
            _refreshReason |= IconRequestReason.ThemeChanged;
        }
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        {
            _needsRefresh = true;
            _refreshReason |= IconRequestReason.Loaded;
            if (changedTheme)
            {
                _refreshReason |= IconRequestReason.ThemeChanged;
            }
            ...
            if (changedScale)
            {
                _refreshReason |= IconRequestReason.ScaleChanged;
            }
        }
    ...
    {
        if (!IsLoaded || !_needsRefresh || SourceKey is not { } sourceKey || _sourceRequested is not { } sourceRequested)
        {
            return;
        }

        _needsRefresh = false;
        var reason = _refreshReason;
        _refreshReason = IconRequestReason.None;
    ...
    private static void OnSourceKeyPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        self._resolvedSourceExpectsImage = false;

        if (e.NewValue is null)
        {
            self._needsRefresh = false;
            self._refreshReason = IconRequestReason.None;
            self.Source = null;
            return;
        }

        self._needsRefresh = true;
        self._refreshReason |= IconRequestReason.SourceChanged;
        ...
        IconRequestReason reason)
    {
        ...
        try
        {
            ...
            iconBox._presentation.SetResolvedSource(eventArgs.Value);
            iconBox._resolvedSourceExpectsImage = eventArgs.ExpectsImageSource;
        ...
        catch (Exception ex)
        {
            ...
            if (requestVersion == iconBox._requestVersion)
            {
                ...
                iconBox._needsRefresh = iconBox.SourceKey is not null;
                iconBox._refreshReason |= IconRequestReason.Retry;
            }
    ...
    private void UpdatePresentedSource()
    {
        ...
        var preferFallback = resolvedSource is null
            || (PreferFallbackSourceForFontIcons && _resolvedSourceExpectsImage && resolvedSource is FontIconSource)
