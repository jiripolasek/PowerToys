...
internal sealed class CachedIconSourceProvider : IIconSourceProvider
{
        ...
        IIconRequestDemand? demand)
    {
        ...
        try
        {
            ...
            if (_loader.TryLoadGlyph(icon.Icon, icon.FontFamily, _iconSize, scale, out var glyph))
            {
        ...
        IIconProtocolProcessor? protocolProcessor)
    {
        ...
        try
        {
            return FontIconGlyphClassifier.Classify(iconString) is FontIconGlyphKind.Invalid or FontIconGlyphKind.None
