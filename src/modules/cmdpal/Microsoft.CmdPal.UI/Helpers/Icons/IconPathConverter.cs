...
internal static partial class IconPathConverter
{
    ...
    public static Task<IconSource> CompleteIconSourceCreationAsync(PreparedIcon icon) =>
        icon.Kind switch
        {
            PreparedIconKind.Binary => CreateBinaryIconSourceAsync(icon.SoftwareBitmap!),
            PreparedIconKind.SvgData => CreateSvgIconSourceAsync(icon.SvgData!, icon.TargetSize),
            _ => Task.FromResult<IconSource>(CreateEmptyIconSource()),
        };
    ...
    private static async Task<IconSource> CreateBinaryIconSourceAsync(SoftwareBitmap softwareBitmap)
    {
        try
        {
            ...
            return new ImageIconSource { ImageSource = bitmapSource };
        }
        catch
        {
            return new ImageIconSource();
